...
def llm_generate(model_config, prompts, max_new_tokens=200, temperature=0.7, 
    ...
    Parameters:
      ...
      - temperature (float, optional): The temperature for sampling-based strategies. Lower values make responses more deterministic. Default is 0.7.
      - search_strategy (str, optional): The decoding strategy to use. Options:
        - `"top_k"`: Samples from the top `k` most likely tokens.
        - `"top_p"`: Uses nucleus sampling (top-p filtering).
        - `"contrastive"`: Uses contrastive search with a fixed `top_k=4` and `penalty_alpha=0.6`.
        - `"beam_search"`: Uses beam search with `num_beams > 1`.
        - `"deterministic"`: Disables sampling (`do_sample=False`), sets `temperature=0.0`, and uses `num_beams=1` for fully deterministic outputs.
      - top_k (int, optional): The number of top tokens to sample from when `search_strategy="top_k"`. Default is 50.
      - top_p (float, optional): The cumulative probability threshold for nucleus sampling (`search_strategy="top_p"`). Default is 0.9.
      - num_beams (int, optional): The number of beams for beam search (`search_strategy="beam_search"`). Default is 1 (greedy decoding).
    ...
    Notes:
      - When using `"deterministic"`, the model will generate identical outputs for the same input.
      - API-based models (e.g., OpenAI, Gemini) ignore most decoding parameters and rely on their own implementations.
      - If the model or tokenizer is not properly initialized in `model_config`, an error message is returned.
            ...
            try:
                ...
                for prompt in ([prompts] if not is_batch else prompts):
                    ...
                    response = model_config.client.chat.completions.create(
                        model=model_config.model_str,
                        messages=full_messages,
                        temperature=temperature if search_strategy != "deterministic" else 0.0,  # Set temp to 0 for determinism
                        max_tokens=max_new_tokens
                    )
                    response_text = response.choices[0].message.content.strip()
                    responses.append(clean_response(response_text, prompt))  # Apply `clean_response`
