...
def download_and_extract_nuclei_data(target_dir=Path("./data")):
    """Downloads and extracts NucleiData.zip from Hugging Face and removes the zip file if not already present."""
    url = "https://huggingface.co/datasets/hobbes99/NucleiData/resolve/main/nuclei_data.zip"
    zip_path = target_dir / "nuclei_data.zip"
    data_path = target_dir / "nuclei_data"
    ...
    # Delete the zip file
    zip_path.unlink()
    print(f"Extraction complete. Data available in {target_dir}")
...
def display_yolo_predictions(yaml_file, model, num_samples=3, imgsz=640, conf=0.25,
                              indices=None, figsize=(6, 6), show_confidence=True, 
    ...
    if isinstance(val_images_path, str):
        ...
        # sort them for consistent selection across platforms
        val_images_path = sorted((Path(val_images_path)).iterdir())
...
def prepare_penn_fudan_yolo(target_dir: Path, seed=42):
    ...
    train_indices, _ = train_test_split(indices, test_size=0.2, random_state=seed)
